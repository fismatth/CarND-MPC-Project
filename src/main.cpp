		...
		if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
			...
			if (s != "") {
				...
				if (event == "telemetry") {
					...
					// transform points into vehicle coordinate system
					for (int i = 0; i < ptsx.size(); ++i)
					{
						double x_map = ptsx_eigen(i);
						double y_map = ptsy_eigen(i);
						// inverse operation (world -> vehicle): do translation first
						double x_t = x_map - px;
						double y_t = y_map - py;
						// rotate by -psi
						double x_veh = x_t * cos(-psi) - y_t * sin(-psi);
						double y_veh = x_t * sin(-psi) + y_t * cos(-psi);
						ptsx_eigen(i) = x_veh;
						ptsy_eigen(i) = y_veh;
					}
					...
					// px = py = psi = 0 (as we transformed into vehicle coordinate system)
					double cte = polyeval(coeffs, 0.0);
					double epsi = - atan(polyeval_derivative(coeffs, 0.0));
					Eigen::VectorXd state(6);
					state << 0.0, 0.0, 0.0, v, cte, epsi;

					std::vector<double> actuators = mpc.Solve(state, coeffs);
					double steer_value = -actuators[0] / deg2rad(25.0);
					...
					msgJson["mpc_x"] = mpc.x_predicted_;
					msgJson["mpc_y"] = mpc.y_predicted_;

					//Display the waypoints/reference line
					vector<double> next_x_vals(MPC::N);
					vector<double> next_y_vals(MPC::N);

					for (int i = 0; i < MPC::N; ++i)
					{
						double x = mpc.x_predicted_[i];
						next_x_vals[i] = x;
